...
def evaluate_heuristic(heuristic_class, environment, parameter_dict, number_of_episodes):
    heuristic_agent = heuristic_class(**parameter_dict)
    reward_mean, reward_std = evaluate_agent(heuristic_agent, environment, number_of_episodes)
...
def evaluate_heursitic_grid(
    ...
    parameter_list = [dict(zip(parameter_dict, v)) for v in itertools.product(*parameter_dict.values())]
    ...
    if result_path is not None and overwrite is False and os.path.exists(result_path):
        ...
        if not results.loc[0]["episodes"] != number_of_episodes:
            parameter_list = parameter_list[len(results):] # only evaluate new parameters
        else:
            raise ValueError("The result file already exists and has a different number of episodes. Set overwrite=True to overwrite the file.")
    else:
    ...
    with tqdm(total=len(jobs)) as pbar:
        for job in jobs:
            ...
            results.loc[len(results)] = [parameters[k] for k in parameter_keys] + [reward_mean, reward_std, number_of_episodes]
            if result_path is not None:
                results.to_csv(result_path, index=False)
            pbar.set_description(f"{' '.join([f'{k}: {v}' for k,v in parameters.items()])} - Avg: {reward_mean:.2f}, Std: {reward_std:.2f}")
    pool.close()
