...
Usage:
    python check_traffic.py                    # Analyze only, print statistics
    python check_traffic.py --fix              # Analyze and save fixed data
    python check_traffic.py --fix --output output.csv  # Save to custom file
"""
...
def analyze_and_fix_traffic(
    ...
    for idx, row in tqdm(traffic_df.iterrows(), total=len(traffic_df), desc="Analyzing trips"):
        stats['total'] += 1
    ...
    print(f"Kept (direct match):     {kept_count:>10,} ({100*kept_count/stats['total']:.2f}%)")
    print(f"  Correctly oriented:    {stats['correct']:>10,} ({100*stats['correct']/stats['total']:.2f}%)")
    print(f"  Reversed (fixed):      {stats['reversed']:>10,} ({100*stats['reversed']/stats['total']:.2f}%)")
    print(f"Discarded:               {stats['discarded']:>10,} ({100*stats['discarded']/stats['total']:.2f}%)")
    print("="*60)
    ...
    if fix:
        ...
        print(f"Discarded {stats['discarded']} trips that could not be determined by direct match.")
        return result_df
    else:
...
def main():
    ...
    parser.add_argument(
        "--fix",
        action="store_true",
        help="Fix reversed paths and save to output file"
    )
    parser.add_argument(
        "--output", "-o",
        type=str,
    ...
    fixed_df = analyze_and_fix_traffic(
        traffic_df,
        edges_df,
        regions_df,
        fix=args.fix
