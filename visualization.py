...
edge_labels_dict = {'edge_labels': None, 'label_pos': 0.5, 'font_size': 10, 'font_color': 'k', 'horizontalalignment': 'center', 
...
def only_graph_structure(g: nx.Graph, my_node_dict: dict={}, my_edge_dict: dict={}, 
                         my_node_label_dict: dict={}, my_edge_label_dict: dict={}) -> [dict, dict, dict, dict]:
    ...
    new_node_dict, new_edge_dict, new_node_label_dict, new_edge_label_dict = update_multiple_dicts(g=g, str_list=str_list, dict_list=dict_list)
    return [new_node_dict, new_edge_dict, new_node_label_dict, new_edge_label_dict]

def only_edge_colors(g: nx.Graph, ax: plt.axes, use_cmap=False, my_edge_dict: dict={}) -> dict:
    ...
    if use_cmap:
        ...
        new_edge_dict = update_dict(d=my_edge_dict, my_dict={'edge_color': edge_colors, 'edge_vmax': num_states})
        ...
        new_edge_dict = update_dict(d=my_edge_dict, my_dict={'edge_color': edge_colors, 'edge_cmap':None, 'edge_vmin':0, 'edge_vmax': None})
...
def general_plot(g: nx.Graph, layout='planar', with_color: bool=False, use_cmap: bool=False, with_edge_labels: bool=False, with_volumes: bool=False, 
                 my_node_dict: dict={}, my_edge_dict: dict={}, my_node_label_dict: dict={}, my_edge_label_dict: dict={}, title: bool=None, 
                 show_plot: bool=True, save_plot: bool=False, filename: str=None, return_stuff: bool=False) -> None: 
    
    fig, ax, pos, g = plot_prepare(g=g, layout=layout)
    new_node_dict, new_edge_dict, new_node_label_dict, new_edge_label_dict = only_graph_structure(g=g, my_node_dict=my_node_dict, my_edge_dict=my_edge_dict, 
                                                                                                  my_node_label_dict=my_node_label_dict)

    # overwrite the respective dicts based on the desired inputs
    if with_color:
        new_edge_dict = only_edge_colors(g=g, ax=ax, use_cmap=use_cmap, my_edge_dict=new_edge_dict)
    if with_volumes:
        new_edge_dict = only_volumes(g=g, my_edge_dict=new_edge_dict)
    if with_edge_labels:
        new_edge_label_dict = only_edge_labels(g=g, my_edge_label_dict=my_edge_label_dict)
    ...
    plot_ending(title=title, show_plot=show_plot, save_plot=save_plot, filename=filename)
...
def plot_only_graph_structure(g: nx.Graph, layout='planar', my_node_dict: dict={}, my_edge_dict: dict={}, my_node_label_dict: dict={}, 
                              title=None, show_plot=True, save_plot=False, filename=None, return_stuff=False) -> None:
    
    fig, ax, pos, g, nnd, ned, nnld, _ = general_plot(g=g, layout=layout, my_node_dict=my_node_dict, my_edge_dict=my_edge_dict, 
                                                      my_node_label_dict=my_node_label_dict, title=title, show_plot=show_plot, 
                                                      save_plot=save_plot, filename=filename, return_stuff=True)
    return [fig, ax, pos, g, nnd, ned, nnld] if return_stuff else None

def plot_only_states(g: nx.Graph, layout='planar', use_cmap=False, my_node_dict: dict={}, my_edge_dict: dict={}, 
                     my_node_label_dict: dict={}, title=None, show_plot=True, save_plot=False, filename=None, return_stuff=False) -> None:

    fig, ax, pos, g, nnd, ned, nnld, _ = general_plot(g=g, layout=layout, with_color=True, use_cmap=use_cmap, my_node_dict=my_node_dict, 
                                                      my_edge_dict=my_edge_dict, my_node_label_dict=my_node_label_dict, title=title, 
                                                      show_plot=show_plot, save_plot=save_plot, filename=filename, return_stuff=True)
    return [fig, ax, pos, g, nnd, ned, nnld] if return_stuff else None

def plot_states_with_edge_labels(g: nx.Graph, layout='planar', use_cmap=False, my_node_dict: dict={}, my_edge_dict: dict={}, 
                                 my_node_label_dict: dict={}, my_edge_label_dict: dict={}, with_color: bool=True, 
                                 title=None, show_plot=True, save_plot=False, filename=None, return_stuff=False) -> None:
    fig, ax, pos, g, nnd, ned, nnld, neld = general_plot(g=g, layout=layout, with_color=True, use_cmap=use_cmap, with_edge_labels=True, 
                                                         my_node_dict=my_node_dict, my_edge_dict=my_edge_dict, my_node_label_dict=my_node_label_dict, 
                                                         my_edge_label_dict=my_edge_label_dict, title=title, show_plot=show_plot, save_plot=save_plot, 
                                                         filename=filename, return_stuff=True)
    return [fig, ax, pos, g, nnd, ned, nnld, neld] if return_stuff else None


def plot_only_volumes(g: nx.Graph, layout='planar', my_node_dict: dict={}, my_edge_dict: dict={}, my_node_label_dict: dict={}, 
                      my_edge_label_dict: dict={}, title=None, show_plot=True, save_plot=False, filename=None, return_stuff=False) -> None:
    
    fig, ax, pos, g, nnd, ned, nnld, _ = general_plot(g=g, layout=layout, with_volumes=True, my_node_dict=my_node_dict, my_edge_dict=my_edge_dict, 
                                                         my_node_label_dict=my_node_label_dict, my_edge_label_dict=my_edge_label_dict, title=title, 
                                                         show_plot=show_plot, save_plot=save_plot, filename=filename, return_stuff=True)
    return [fig, ax, pos, g, nnd, ned, nnld, _] if return_stuff else None
    

def plot_states_labels_and_volumes(g: nx.Graph, layout='planar', with_color: bool=True, use_cmap: bool=False, with_edge_labels: bool=True, 
                                   my_node_dict: dict={}, my_edge_dict: dict={}, my_node_label_dict: dict={}, my_edge_label_dict: dict={}, 
                                   title=None, show_plot=True, save_plot=False, filename=None, return_stuff=False) -> None:
    
    fig, ax, pos, g, nnd, ned, nnld, neld = general_plot(g=g, layout=layout, with_color=True, use_cmap=use_cmap, with_edge_labels=True, 
                                                         with_volumes=True, my_node_dict=my_node_dict, my_edge_dict=my_edge_dict, 
                                                         my_node_label_dict=my_node_label_dict, my_edge_label_dict=my_edge_label_dict, 
                                                         title=title, show_plot=show_plot, save_plot=save_plot, filename=filename, return_stuff=True)
    return [fig, ax, pos, g, nnd, ned, nnld, neld] if return_stuff else None
