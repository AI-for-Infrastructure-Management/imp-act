...
def export_country(args):
    ...
    G_reduced_2 = remove_nodes_with_degree_one_below_threshold(G_reduced.copy(), args.pruning_threshold)

    # Merge nodes with only two edges again after removing edges below threshold
    G_reduced_3, new_edge_info = remove_nodes_and_merge_edges(G_reduced_2.copy(), cleanup=True)

    # Add position data for nodes to graph
    position_dict = {id: {"position_x":pos[0], "position_y":pos[1]} for id, pos in pos_filtered.items()}
    nx.set_node_attributes(G_reduced_3, position_dict)

    #print(G_reduced_3.nodes()[106886])

    nx.write_graphml_lxml(G_reduced_3, f'{country_output_path.absolute()}/graph.graphml')
    ...
    for edge in G_reduced_3.edges():
        ...
        no_segments = int(np.ceil(G_reduced_3.edges[edge]['Distance'] / args.segment_length))

        # linear interpolation of coordinates
        x_coordinates = np.linspace(node_a['position_x'], node_b['position_x'], no_segments)
        y_coordinates = np.linspace(node_a['position_y'], node_b['position_y'], no_segments)

        for x,y in zip(x_coordinates, y_coordinates):
            segments.append((edge[0], edge[1], x, y, args.segment_capacity, travel_time))

        total_number_of_segments += no_segments
        # print(f'Edge {edge} has {no_segments} segments')

    segments_df = pd.DataFrame(segments, columns=['Network_Node_A_ID', 'Network_Node_B_ID', 'position_x', 'position_y', 'capacity', 'travel_time'])
    # save
    ...
    # export to csv
    truck_traffic_df_filtered.to_csv(f'{country_output_path.absolute()}/traffic_full.csv', index=False)
    
    # drop everything except origin, destination, Traffic_flow_trucks_2019
    truck_traffic_df_filtered = truck_traffic_df_filtered[['origin', 'destination', 'Traffic_flow_trucks_2019']]

    # rename Traffic_flow_trucks_2019 to volume
    truck_traffic_df_filtered = truck_traffic_df_filtered.rename(columns={'Traffic_flow_trucks_2019': 'volume'})

    # export to csv
    truck_traffic_df_filtered.to_csv(f'{country_output_path.absolute()}/traffic.csv', index=False)
...
def main(args):
    ...
    # Plotting the network
    plot_network(G, pos, f'Europe (N: {len(G.nodes)}, E: {len(G.edges)})', args.output_dir)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--segment_length', '-sl', type=float, default=10.0, help='Length of a segment in km')
    parser.add_argument('--segment_capacity', '-sc', type=float, default=9e6, help='Capacity of a segment in trucks per year')
    parser.add_argument('--segment_speed', '-ss', type=float, default=100.0, help='Travel speed on a segment in km/h')
    parser.add_argument('--pruning_threshold', '-p', type=float, default=10.0, help='Threshold for pruning edges in km')
    parser.add_argument('--data-dir', '-d', type=str, default='environments/graph/data')
    parser.add_argument('--output-dir', '-o', type=str, default='environments/graph/output')
    ...
    required_files = [truck_traffic_file, nuts_regions_file, nodes_file_name, edges_file_name]
    for file in required_files:
        if not os.path.exists(os.path.join(args.data_dir, file)):
            raise ValueError(f'Data file {file} does not exist in {args.data_dir}. Please download the data from {data_url} and extract it to {args.data_dir}')
